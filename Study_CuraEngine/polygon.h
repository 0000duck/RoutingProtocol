...
public:
	...
	Polygons unionPolugons(const Polygons& other)const
	{
		Polygons ret;
		ClipperLib::Clipper clipper;
		clipper.AddPolygons(polygons, ClipperLib::ptSubject);
	...
	void applyMatrix(const PointMatrix& matrix)
	{
		for (unsigned int i = 0; i < polygons.size(); i++)
		{
			...
			{
				polygons[i][j] = matrix.apply(polygons[j][j]);
			}
		}
	}
};
...
public:
	...
	AABB()
		:mmin(LLONG_MIN,LLONG_MIN),mmax(LLONG_MIN,LLONG_MIN)
	{}
	AABB(Polygons polys)
		:mmin(LLONG_MIN, LLONG_MIN), mmax(LLONG_MIN, LLONG_MIN)
	{
	...
	{
		mmin = Point(LLONG_MAX, LLONG_MAX);
		mmax = Point(LLONG_MIN, LLONG_MIN);
		for (unsigned int i = 0; i < polys.size(); i++)
		{
			for (unsigned int j = 0; j < polys[i].size(); j++)
			{
				if (mmin.X > polys[i][j].X)mmin.X = polys[i][j].X;
				if (mmin.Y > polys[i][j].Y) mmin.Y = polys[i][j].Y;
				if (mmax.X < polys[i][j].X) mmax.X = polys[i][j].X;
				if (mmax.Y < polys[i][j].Y) mmax.Y = polys[i][j].Y;
			}
		}
	}
