...
Slicer::Slicer(OptimizedVolume * ov, int32_t initial, int32_t thickness, bool keepNoneClosed, bool extensiveStitching)
{
	...
	//函数的前大半部分计算了三角面上被切割得到的线段，
	//这些线段只有起点和终点，之间没有什么联系。
	//后小部分使用makepolygon函数将线段们连接成多边形。
	double t = getTime();
...
void Slicer::dumpSegmentsToHTML(const char * filename)
{
	...
	float scale = max(modelSize.x, modelSize.y) / 1500;
...
//把线段们连接成多边形
//首先选取一个线段，然后通过它所在的三角面，找到和其相邻的三角面，
//在找到相邻三角面上的线段，查看其是否匹配，依次进行下去，
//最后会回到最初的那个三角面上，连接结束。
