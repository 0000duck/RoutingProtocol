...
public:
	...
	SimpleFace(Point3& v0, Point3& v1, Point3& v2)
	{
...
public:
	...
	{
		faces.push_back(SimpleFace(v0, v1, v2));//push_back()在vector类中作用为在vector尾部加入一个数据
	}
...
public:
	...
	Point3 mmin()
	{
		Point3 ret = volumes[0].mmin();
		for (unsigned int i = 0; i<volumes.size(); i++)
		{
			Point3 v = volumes[i].mmin();
			SET_MIN(ret.x, v.x);
			SET_MIN(ret.y, v.y);
			SET_MIN(ret.z, v.z);
		}
	...
	Point3 mmax()
	{
		Point3 ret = volumes[0].mmax();
		for (unsigned int i = 0; i<volumes.size(); i++)
		{
			Point3 v = volumes[i].mmax();
			SET_MAX(ret.x, v.x);
			SET_MAX(ret.y, v.y);
			SET_MAX(ret.z, v.z);
		}
